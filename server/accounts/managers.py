...
class DeviceManager(models.Manager.from_queryset(DeviceQuerySet)):
  ...
  def update_from_payload(self, *, device_id: str, payload: dict[str, Any]) -> tuple['Device', bool]:
...
class SocialAccountQuerySet(models.QuerySet):
  ...
  def for_identity(self, provider: str, provider_user_id: str) -> 'SocialAccountQuerySet':
...
class UsernameChangeQuerySet(models.QuerySet):
  ...
  def active_for(self, user: 'CustomUser', *, at: datetime | None = None) -> 'UsernameChangeQuerySet':
